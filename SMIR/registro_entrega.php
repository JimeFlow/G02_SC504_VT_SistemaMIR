<?php
// registro_entrega.php
require_once 'config/db.php';

// Consulta para obtener registros de entregas junto con datos de empleado y material
$query = "SELECT ent.Id_Entrega,
                 per.UPI,
                 per.NOMBRE || ' ' || per.APELLIDOS AS NOMBRE_EMPLEADO,
                 mat.Nombre AS NOMBRE_MATERIAL,
                 ent.Cantidad_Restante AS CANTIDAD,
                 ent.Fecha AS FECHA_ENTREGA,
                 ent.Destinatario AS ENCARGADO
          FROM FIDE_ENTREGAS_TB ent
          JOIN FIDE_PERSONAL_TB per ON ent.UPI = per.UPI
          JOIN FIDE_MATERIALES_TB mat ON ent.Id_Material = mat.Id_Material";
...
<head>
    ...
    <title>SMIR - Registro Entrega</title>
    ...
    <div class="content">
        ...
        <div class="card mt-4">
            <div class="card-header bg-primary text-white">
                <h5><i class="fas fa-box"></i> Registrar Entrega</h5>
            </div>
            <div class="card-body">
                <form action="procesar_entrega.php" method="POST">
                    <div class="row">
                        <div class="col-md-6">
                            <label class="form-label">Empleado (UPI)</label>
                    ...
                    <button type="submit" class="btn btn-success mt-3"><i class="fas fa-save"></i> Registrar Entrega</button>
                </form>
            </div>
        </div>
        <!-- Tabla de Registros de Entregas -->
        <div class="card mt-4">
            <div class="card-header bg-dark text-white">
                <h5><i class="fas fa-list"></i> Registros de Entregas</h5>
            </div>
            <div class="card-body">
                <table id="tablaEntregas" class="table table-striped">
                    ...
                    <tbody>
                        <?php if(!empty($entregas)): ?>
                            <?php foreach($entregas as $reg): ?>
                                <tr>
                                    <td><?php echo htmlspecialchars($reg['UPI']); ?></td>
                                    <td><?php echo htmlspecialchars($reg['NOMBRE_EMPLEADO']); ?></td>
                                    <td><?php echo htmlspecialchars($reg['NOMBRE_MATERIAL']); ?></td>
                                    <td><?php echo htmlspecialchars($reg['CANTIDAD']); ?></td>
                                    <td><?php echo htmlspecialchars($reg['FECHA_ENTREGA']); ?></td>
                                    <td><?php echo htmlspecialchars($reg['ENCARGADO']); ?></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr><td colspan="6">No hay registros de entrega.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
