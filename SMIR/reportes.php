...
// Ejemplo de consulta para obtener datos de inventario por bodega
// Suponiendo que la tabla FIDE_BODEGAS_TB tiene el campo "Sucursal" y que se puede contar la cantidad de materiales en FIDE_INVENTARIO_TB
$query = "SELECT bod.Sucursal AS Bodega, COUNT(*) AS Cantidad
...
<body>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            let ctx = document.getElementById('graficoReportes').getContext('2d');
            let chart;
            
            // Datos dinámicos para inventario: extraídos de la consulta en PHP
            let datosInventario = <?php echo json_encode($datosInventario); ?>;
            
            // Datos estáticos para empleados (puedes reemplazar con una consulta similar)
            let datosEmpleados = [
                { turno: "A", cantidad: 250 },
                { turno: "B", cantidad: 285 },
                { turno: "C", cantidad: 148 }
            ];
            ...
            function generarGrafico(tipo) {
                ...
                if (tipo === "inventario") {
                    ...
                    etiquetas = datosEmpleados.map(d => `Turno ${d.turno}`);
                    valores = datosEmpleados.map(d => d.cantidad);
